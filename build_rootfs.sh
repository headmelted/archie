...
sudo chroot rootfs /bin/bash -c "dpkg --add-architecture $COBBLER_ARCH";

echo "Configuring dpkg"
sudo chroot rootfs /bin/bash -c "dpkg --configure -a";

echo "Updating APT";
sudo chroot rootfs /bin/bash -c "apt-get update -yq";
